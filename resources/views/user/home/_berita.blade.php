<div class="flex flex-col lg:flex-row gap-5 py-10">
    <div class="w-full flex flex-col gap-4">
        ...
        @foreach ($berita as $item)
            ...
            <div class="flex flex-col sm:flex-row gap-4 border-b-2 pb-4">
                <img src="{{ asset('image/' . $item->image) }}" alt="" class="object-cover w-full sm:w-[25%]">
                <div>
                    ...
                    <h1 class="capitalize font-semibold my-2">Kategori : {{ $item->beritaKategori->kategori }}</h1>
    ...
    <div class="w-full lg:w-[40%]">
        ...
        <div>
            <h1 class="text-base md:text-xl lg:text-2xl  font-semibold text-center">Info Penting</h1>
            <ul class="flex flex-col">
                ...
                @foreach ($infoPenting as $item)
                    @php
                        ...
                        $deskripsi = strip_tags($deskripsi, '<br>');
                    @endphp
                    <li class="bg-white my-2 shadow-lg list-none" x-data="accordion({{ $loop->iteration }})">
                        <h2 @click="handleClick()"
                            class="flex flex-row justify-between items-center font-semibold p-3 cursor-pointer">
                            <span class="text-sm xl:text-base line-clamp-2">{{ $item->judul }}</span>
                            <svg :class="handleRotate()"
                                class="fill-current text-main h-6 w-6 transform transition-transform duration-500"
                                viewBox="0 0 20 20">
                                <path
                        ...
                        <div x-ref="tab" :style="handleToggle()"
                            class="border-l-2 border-main overflow-hidden max-h-0 duration-500 transition-all">
                            <p class="p-3 text-gray-900 text-sm xl:text-base line-clamp-3">
                                {!! $deskripsi !!}
                            </p>
                            <a href="/berita/read/{{ $item->slug }}" class="text-main my-2 p-3 font-semibold">Baca Lebih Lanjut</a>
                        </div>
                    </li>
                @endforeach
            ...
            <div class="flex flex-wrap gap-4 mt-4">
                @foreach ($kategoriBerita as $item)
                    <a href="/berita?kategori={{ $item->kategori }}"
                        class="inline-block border rounded-md p-2 hover:bg-main hover:text-white capitalize">{{ $item->kategori }}</a>
                @endforeach
            </div>
        </div>
        <div class="mt-5">
            <h1 class="text-base md:text-xl lg:text-2xl  font-semibold text-center">Berita Favorit</h1>
            <div class="mt-4 flex flex-col gap-4">

                @foreach ($beritaFavorit as $item)
                    @php
                        $date = Carbon\Carbon::parse($item->created_at)
                            ->locale('id')
                            ->isoFormat('D MMMM YYYY');
                    @endphp
                    <a href="/berita/read/{{ $item->slug }}"
                        class="w-full p-2 rounded-md hover:bg-main hover:text-white border inline-block shadow-md line-clamp-3 group">
                        <div class="flex gap-x-2 mb-2">
                            <div class="flex gap-x-2">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512"
