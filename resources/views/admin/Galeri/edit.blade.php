...
@section('container')
    <div class="pt-24 pb-20">
        <h1 class="text-gray-900 dark:text-white font-semibold text-xl">Galeri - Edit Foto</h1>

        @php
            $image = explode("|", $data->image);
        @endphp
        
        <form action="/admin/galeri/edit/{{ $data->slug }}" method="POST" id="my-dropzone" enctype="multipart/form-data" class=" max-w-6xl mx-auto">
            <div id="previewImage" class="flex gap-x-4 my-32 overflow-x-auto">
                @foreach ($image as $item) 
                    <img src="{{ asset('Galeri/'.$item) }}" alt="" class="w-[200px] h-full object-cover">
                @endforeach
            </div>
            @csrf
            <div class="w-full">
